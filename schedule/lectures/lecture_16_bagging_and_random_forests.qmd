...
- Therefore, we can use the OOB samples to estimate the prediction error of the bagged ensemble
  ...
  - Estimate the risk as the average loss over all OOB predictions on the training set:
    ...
    $$
    \widehat{\mathcal{R}}_{\mathrm{OOB}} = \frac{1}{n} \sum_{j=1}^n L(Y_j, \hat f^{(\mathrm{OOB})}(X_j))
    $$
  :::
...
- A simple way to reduce correlation between trees is to only consider a random subset of the covariates when making any given split in each tree.
...
- Since decision trees are greedy algorithms that split on the most informative covariates first,
  forcing each split to consider only a random subset of covariates will lead to more diverse trees in the ensemble,
...
- Draw a bootstrap sample $\mathcal{D}_i$ (i.e. sample of size $n$, with replacement) from $\mathcal{D}$
- Train $\hat{f}_{\mathcal{D}_i}(X)$ (e.g. the decision tree) on $\mathcal{D}_i$, using the following modification:
  - At each split in the tree, randomly select $k = \sqrt{p}$ covariates from the full set of $p$ covariates
  - Choose the best split among only these $k$ covariates
...
- Random forests further reduce correlation between trees in the ensemble by only considering a random subset of covariates at each split,
