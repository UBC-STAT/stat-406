...
- Let's begin with ridge regression on 1D data, with $d$-Fourier basis functions:
  ...
  \begin{align*}
  \hat f_\mathrm{ridge}(X) = \boldsymbol \phi(X)^\top \hat \beta_\mathrm{ridge}, \qquad \hat \beta_\mathrm{ridge} = \left( \boldsymbol \Phi^\top \boldsymbol \Phi + \lambda \boldsymbol I \right)^{-1} \boldsymbol \Phi^\top \boldsymbol Y
  \end{align*}
...
We can verify this by starting with the right-hand side, and using the SVD of $\boldsymbol \Phi = \boldsymbol U \boldsymbol D \boldsymbol V^\top.$
...
- It turns out that, with a suitable choice of frequencies, the kernel function converges to a limit as $d \to \infty$. The kernel is an **average** over $d/2$ terms:

  \begin{align*}
  k(x, x') = \frac{1}{d} \sum_{j=1}^{d/2} \left[ \cos(W_j x) \cos(W_j x') + \sin(W_j x) \sin(W_j x') \right],
  \end{align*}

  and, as $d \to \infty$, this average converges to an **expectation**:

  \begin{align*}
  k(x, x') \;\xrightarrow{d \to \infty}\; \mathbb E_{W} \left[ \cos(W x) \cos(W x') + \sin(W x) \sin(W x') \right]
  = e^{-\frac{1}{2 \gamma^2} (x - x')^2}.
  \end{align*}

- This limiting kernel is known as the **Gaussian** (or **radial basis function, RBF**) kernel. It corresponds to a basis regressor with **infinitely many** basis functions.

::: {.callout-note collapse="true" title="(Advanced!) Why does the Limiting Kernel Converge?"}

To take $d \to \infty$, imagine that we make our basis expansions **random**, rather than fixed:
...
That is, we replace the fixed frequencies $2 \pi, 4 \pi, \ldots, 2 \pi \frac{d}{2}$ with random frequencies $W_j$ drawn from $\mathcal{N}(0, 1/\gamma^2)$. The kernel function then becomes

\begin{align*}
k(x, x') = \frac{1}{d} \sum_{j=1}^{d/2} \left[ \cos(W_j x) \cos(W_j x') + \sin(W_j x) \sin(W_j x') \right].
\end{align*}

For a fixed $x$ and $x'$, the summation terms are i.i.d. random variables (after fixing $x$ and $x'$, the only random quantity is $W_j$, and the $W_j$ are i.i.d.). By the **law of large numbers**, the average converges to its expectation as $d \to \infty$:

\begin{align*}
k(x, x') &\to \mathbb E_{W \sim \mathcal N(0, 1/\gamma^2)} \left[ \cos(W x) \cos(W x') + \sin(W x) \sin(W x') \right] \\
&= e^{-\frac{1}{2 \gamma^2} (x - x')^2}.
\end{align*}
