...
- Consider our OLS estimator:
  ...
  $$
  \hat f_{\text{OLS}}(X) = X^\top \hat{\beta}_{\text{OLS}}, \qquad \hat{\beta}_{\text{OLS}} = (\boldsymbol{X}^\top \boldsymbol{X})^{-1} \boldsymbol{X}^\top \boldsymbol{Y}
  $$
...
- It turns out that we can! With some ugly linear algebra, we can show that the LOO-CV predictions for OLS can be computed as:
  ...
  $$
  \hat{Y}^{-i}(X_i) = \frac{\hat{Y}_i - H_{ii} Y_i}{1 - H_{ii}},
  $$
