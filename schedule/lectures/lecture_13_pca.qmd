...
- Imagine that we have a linear model $\hat f(X) = X^\top \hat \beta$.
...
- Then, we can rewrite our linear model as
  ...
  $$ \hat f(X) = X^\top \boldsymbol Q \boldsymbol Q^\top \hat \beta = (X')^\top \underbrace{(\boldsymbol Q^\top \hat \beta)}_{\hat \beta'}. $$
