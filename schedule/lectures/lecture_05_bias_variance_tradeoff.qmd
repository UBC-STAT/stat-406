...
1. decided that **risk** ($\mathbb E[ L(Y, \hat f_\mathcal{D}(X)) ]$) is the metric to quantify ``goodness'' of a learned model $\hat f_\mathcal{D}$, and
...
Given a learned model $\hat f_\mathcal{D}: \mathcal{X} \to \mathcal{Y}$ (the hat denotes that the model is estimated from data, and the subscript $\mathcal D$ emphasizes that it was estimated using the particular training set $\mathcal D$),
recall our definition of **risk** from the previous lecture:

$$ \mathcal R = \mathbb E[ L(Y, \hat f_\mathcal{D}(X)) ] = \mathbb E\left[ \mathbb E\left[ L(Y, \hat f_\mathcal{D}(X)) \mid \mathcal D \right] \right]. $$

This lecture will focus specifically on the square loss (though the ideas will apply to other losses):

$$ \mathcal R = \mathbb E\left[ (Y - \hat f_\mathcal{D}(X))^2 \right] $$

Crucially, risk is an expectation over *all sources of randomness*, including:

- the training data $\mathcal D = \{(X_i, Y_i)\}_{i=1}^n \overset{\text{i.i.d.}}{\sim} \mathbb{P}_{X,Y}$
- the test data $(X, Y) \sim \mathbb{P}_{X,Y}$
...
For any given set of covariates $X$, there is a distribution $\mathbb{P}_{Y \mid X}$ over possible labels.
...
Our learned model $\hat f_\mathcal{D}$ is a random variable because it depends on the (random) training data $\mathcal D$.
We can consider the **expected model**:

$$ \bar{f}(X) = \mathbb E[ \hat f_\mathcal{D}(X) \mid X ]. $$
...
- More specifically, we are going to add and subtract a very specific quantity, the expected label $\bar{Y}(X)$,
  within the square loss portion of risk:
    ...
    $$ \mathcal R = \mathbb E\left[ (Y - \hat f_\mathcal{D}(X))^2 \right] = \mathbb E\left[ \left( Y \overbrace{- \bar{Y}(X) + \bar{Y}(X)}^{= 0} - \hat f_\mathcal{D}(X) \right)^2 \right] $$

- So far, this is dumb. But watch what happens when we group terms...

    $$ \mathcal R = \mathbb E\left[ \left( \underbrace{(Y - \bar{Y}(X))}_{A} + \underbrace{(\bar{Y}(X) - \hat f_\mathcal{D}(X))}_{B} \right)^2 \right] $$
...
- ... and then expand the square into $A^2 + 2AB + B^2$. After simplifying...
    ...
    <details>
      ...
      Consider $\mathbb E\left[ AB \right] = \mathbb E\left[ (Y - \bar{Y}(X))(\bar{Y}(X) - \hat f_\mathcal{D}(X)) \right]$.
      Applying the tower property, we have that:

      $$ \mathbb E\left[ AB \right] = \mathbb E\left[ \mathbb E\left[ (Y - \bar{Y}(X))(\bar{Y}(X) - \hat f_\mathcal{D}(X)) \mid X, \mathcal D \right] \right]. $$
      ...
      \begin{align*}
        ...
        &= \mathbb E\left[ \mathbb E\left[ (Y - \bar{Y}(X))(\bar{Y}(X) - \hat f_\mathcal{D}(X)) \mid X, \mathcal D \right] \right] \\
        &= \mathbb E\left[ \left( \mathbb E[ Y \mid X, \mathcal D] - \bar{Y}(X) \right) \left( \bar{Y}(X) - \hat f_\mathcal{D}(X) \right) \right] \\
        &= \mathbb E\left[ (\bar{Y}(X) - \bar{Y}(X)) \left( \bar{Y}(X) - \hat f_\mathcal{D}(X) \right) \right] = 0.
      \end{align*}
    ...
    $$ \mathcal R = \underbrace{\mathbb E\left[ \left( Y - \bar{Y}(X) \right)^2 \right]}_\text{irreducible error} + \underbrace{\mathbb E\left[ \left( \bar{Y}(X) - \hat f_\mathcal{D}(X) \right)^2 \right]}_\text{estimation error} $$
...
1. The first term, the **irreducible error**, quantifies the inherent uncertainty in the response variable.
  Even if we were to perfectly know the conditional distribution $\mathbb{P}_{Y \mid X}$, we would not be able to perfectly predict $Y$ from $X$
  due to some inherent randomness in $Y \mid X$.

2. The second term, the **estimation error**, quantifies how far off our learned model $\hat f_\mathcal{D}$ is from the optimal prediction rule $\bar{Y}(X)$.
...
- The first term, the prediction error, does not depend on our model $\hat f_\mathcal{D}$ at all.
  It is a property of the data distribution $\mathbb{P}_{X,Y}$, and it is non-negative.
- The second term, the estimation error, is non-negative because it is an expectation of a squared term.
  It will be minimized when it equals zero, which happens when $\hat f_\mathcal{D}(X) = \bar{Y}(X)$ for all $X$.
...
$$ \mathbb E\left[ \left( \bar{Y}(X) - \hat f_\mathcal{D}(X) \right)^2 \right]
= \mathbb E\left[ \left( \underbrace{\left( \bar{Y}(X) - \bar{f}(X) \right)}_{C} - \underbrace{\left( \hat f_\mathcal{D}(X) - \bar{f}(X) \right)}_{D} \right)^2 \right]. $$
...
<details>
  ...
  $$ \mathbb E\left[ CD \right] = \mathbb E\left[ \left( \bar{Y}(X) - \bar{f}(X) \right) \left( \bar{f}(X) - \hat f_\mathcal{D}(X) \right) \right] $$

  ... and again apply the tower property (this time only conditioning on $X$)...

  $$ \mathbb E\left[ CD \right] = \mathbb E\left[ \mathbb E\left[ \left( \bar{Y}(X) - \bar{f}(X) \right) \left( \bar{f}(X) - \hat f_\mathcal{D}(X) \right) \mid X \right] \right] $$

  ... then the only term in the inner expectation that is random (after conditioning on $X$) is $\hat f_\mathcal{D}(X)$.
  Since $\bar{f}(X) = \mathbb E[ \hat f_\mathcal{D}(X) \mid X ]$, we have that

  \begin{align*}
    \mathbb E\left[ CD \right]
    &= \mathbb E\left[ \mathbb E\left[ \left( \bar{Y}(X) - \bar{f}(X) \right) \left( \bar{f}(X) - \hat f_\mathcal{D}(X) \right) \mid X \right] \right] \\
    &= \mathbb E\left[ \left( \bar{Y}(X) - \bar{f}(X) \right) \left( \bar{f}(X) - \mathbb E\left[ \hat f_\mathcal{D}(X) \mid X \right] \right)\right] \\
...
$$ \underbrace{\mathbb E\left[ \left( \bar{Y}(X) - \hat f_\mathcal{D}(X) \right)^2 \right]}_\text{estimation error}
= \underbrace{\mathbb E\left[ \left( \bar{Y}(X) - \bar{f}(X) \right)^2 \right]}_\text{squared bias}
+ \underbrace{\mathbb E\left[ \left( \bar{f}(X) - \hat f_\mathcal{D}(X) \right)^2 \right]}_\text{variance}. $$
...
**Variance** captures how dependent our learned model $\hat f_\mathcal{D}$ is on any particular training dataset $\mathcal D$.
...
**Bias**: captures how much "generalizable truth" can actually be captured by our model (hypothesis class and estimation procedure).
...
- Add features
- Use a more complex predictive model/hypothesis class
- (More to come!)
