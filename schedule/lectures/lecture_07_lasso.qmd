...
- As with ridge regression, we can rewrite the constrained optimization problem as a penalized optimization problem:
  ...
  $$ \hat f_\mathrm{lasso}(X) = X^\top \hat\beta_\lambda, \qquad \hat\beta_\lambda = \mathrm{argmin}_{\hat \beta} \frac 1 n  \sum_{i=1}^n (y_i - x_i^T \hat\beta)^2 + \lambda \Vert \hat\beta \Vert_1 $$
