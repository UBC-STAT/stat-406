...
- Given some training data $\mathcal{D} = \{(x_i, y_i)\}_{i=1}^n$ and some loss function $L(Y, \hat f(X))$, the neural network training problem can be written as:

  $$
  \hat \Theta, \hat \beta = \mathrm{argmin}_{\Theta, \beta} \sum_{i=1}^n L\left(y_i, \hat f(x_i; \Theta, \beta)\right)
  $$

  where $\hat f(x; \Theta, \beta)$ is the neural network's prediction for input $x$ given parameters $\Theta$ and $\beta$.
...
For some small tolerance $\varepsilon > 0$ (e.g., $\varepsilon = 10^{-6}$), we can check any/all of the following conditions:

1. **Gradient magnitude**: $\|\nabla f(w^{(k)})\| < \varepsilon$
   - Stop when the gradient is nearly zero (i.e., we are close to a stationary point).

2. **Step size**: $\|w^{(k)} - w^{(k-1)}\| < \varepsilon$
   - Stop when successive iterates are very close to each other.

3. **Function value change**: $|f(w^{(k)}) - f(w^{(k-1)})| < \varepsilon$
...
- Assuming we're using the squared error loss $L(y, \hat f(x)) = (y - \hat f(x))^2$ and a one-hidden-layer neural network, this optimization problem becomes:
...
![Toy illustration of SGD escaping local minima](../figures/sgd.png){width="600px"}
