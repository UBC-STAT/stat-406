...
- Recall from Lecture 2 that this OLS estimator *is* the empirical risk minimizer for the linear hypothesis class under squared loss: the learned predictor $\hat f_\mathrm{OLS}$ is exactly $\hat f_\mathrm{ERM} = \mathrm{argmin}_{\hat f \in \mathcal H} \hat R_{\hat f}$, so $\hat\beta_\mathrm{OLS} = \hat\beta_\mathrm{ERM}$.

- Keep this in mind: by the end of this lecture we will have constructed our first estimator that is *not* the ERM solution.

- Now let's assume that our data are i.i.d. generated according to the linear model
