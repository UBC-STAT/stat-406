...
- $\boldsymbol U$ is $n \times n$ and orthonormal: $\boldsymbol U^\top \boldsymbol U = \boldsymbol U \boldsymbol U^\top = \boldsymbol I$
- $\boldsymbol V$ is $p \times p$ and orthonormal: $\boldsymbol V^\top \boldsymbol V = \boldsymbol V \boldsymbol V^\top = \boldsymbol I$
- $\boldsymbol D$ is rectangular $n \times p$, diagonal, and nonnegative (contains the singular values)

- Using the SVD, we can rewrite the covariance of OLS as:

$$\text{Cov}(\hat{\beta}_\mathrm{OLS}) = \sigma^2 (\boldsymbol X^\top \boldsymbol X)^{-1} = \sigma^2 (\boldsymbol V \boldsymbol D^\top \boldsymbol D \boldsymbol V^\top)^{-1} = \sigma^2 \boldsymbol V (\boldsymbol D^\top \boldsymbol D)^{-1} \boldsymbol V^\top$$
...
- Using the SVD $\boldsymbol X = \boldsymbol U \boldsymbol D \boldsymbol V^\top$, we can show that:
  $$\hat{\beta}_\lambda = \boldsymbol V (\boldsymbol D^\top \boldsymbol D + \lambda \boldsymbol I)^{-1} \boldsymbol D^\top \boldsymbol U^\top \boldsymbol Y$$
