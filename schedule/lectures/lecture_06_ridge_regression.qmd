...
- Consider the OLS estimator for linear regression:
  ...
  $$ \hat f_\mathrm{OLS}(X) = X^\top \hat{\beta}_\mathrm{OLS}, \qquad \hat{\beta}_\mathrm{OLS} = (\boldsymbol X^\top \boldsymbol X)^{-1} \boldsymbol X^\top \boldsymbol Y. $$
...
- Recall from Lecture 2 that this OLS estimator *is* the empirical risk minimizer for the linear hypothesis class under squared loss: the learned predictor $\hat f_\mathrm{OLS}$ is exactly $\hat f_\mathrm{ERM} = \mathrm{argmin}_{\hat f \in \mathcal H} \; \frac{1}{n} \sum_{i=1}^n L(Y_i, \hat f(X_i))$, so $\hat\beta_\mathrm{OLS} = \hat\beta_\mathrm{ERM}$.

- Keep this in mind: by the end of this lecture we will have constructed our first estimator that is *not* the ERM solution.

- Now let's assume that our data are i.i.d. generated according to the linear statistical model
  $$ Y = f(X) + \varepsilon, \qquad f(X) = X^\top \beta, \qquad \varepsilon \sim \mathcal N(0, \sigma^2), $$
...
$$ \boldsymbol Y = \boldsymbol X \beta + \boldsymbol \varepsilon, \qquad \boldsymbol \varepsilon \sim \mathcal N(\boldsymbol 0, \sigma^2 I). $$

- Let's now analyze the bias and variance of the OLS estimator $\hat f_\mathrm{OLS}$ (through its coefficients $\hat{\beta}_\mathrm{OLS}$) under this model!
...
We have shown that the bias associated with OLS is zero *under the assumption that the linear statistical model is correct*.

In practice, it's very unlikely that the linear statistical model is exactly correct (i.e. the true value of $Y$ may depend on covariates we don't have access to, interaction terms, etc.). In that case (i.e. when the linear statistical model is too simple of an approximation for the ground-truth data-generating process), the bias of OLS is non-zero.
:::
...
- Recall the (thin) SVD from Lecture 1: assuming $n > p$, the $n \times p$ matrix $\boldsymbol X$ factors as $\boldsymbol X = \boldsymbol U \boldsymbol D \boldsymbol V^\top$, where $\boldsymbol U \in \mathbb R^{n \times p}$ has orthonormal columns, $\boldsymbol V \in \mathbb R^{p \times p}$ is orthonormal, and $\boldsymbol D \in \mathbb R^{p \times p}$ is diagonal with nonnegative singular values $d_1, \ldots, d_p$ on its diagonal.
...
- This situation arises when $\boldsymbol X$ is **ill-conditioned** or **nearly rank-deficient**, which happens when we have **multicollinearity**: a linear combination of predictor variables is nearly equal to another predictor variable.
  ...
  - Recall the intuition from Lecture 1: the singular values measure the **effective dimensionality** of $\boldsymbol X$. A near-zero singular value $d_j \approx 0$ means that $\boldsymbol X$ collapses the $p$-dimensional predictor space onto a lower-dimensional subspace — the columns of $\boldsymbol X$ effectively span fewer than $p$ directions.
  - So even though we are estimating $p$ coefficients, the data only constrains them along the few directions with $d_j$ appreciably above zero. In effect, we have **more parameters to fit than effective dimensions to pin them down**, which is exactly why the variance $d_j^{-2}$ blows up.
...
- This yields the ridge predictor $\hat f_\mathrm{ridge}(X) = X^\top \hat\beta_\lambda$, a **different estimator** than $\hat f_\mathrm{OLS}$/$\hat f_\mathrm{ERM}$ arising from a **different estimation procedure**.

:::{.callout-important title="Ridge is not empirical risk minimization"}
Unlike OLS, ridge regression does *not* minimize the empirical risk. For any $\lambda > 0$, the ridge objective $\frac{1}{n}\|\boldsymbol Y - \boldsymbol X\beta\|_2^2 + \lambda\|\beta\|_2^2$ is the empirical risk *plus a penalty term*, so its minimizer $\hat\beta_\lambda$ is generally **not** the empirical risk minimizer $\hat\beta_\mathrm{ERM} = \hat\beta_\mathrm{OLS}$. Indeed, since $\hat f_\mathrm{OLS}$ already achieves the smallest possible empirical risk, we have $\hat R_{\hat f_\mathrm{ridge}} \geq \hat R_{\hat f_\mathrm{OLS}}$ — ridge deliberately accepts higher *training* error in exchange for lower variance. Ridge is therefore our first example of an estimation procedure that departs from ERM.
:::

### Closed Form

- Ridge regression has a closed-form solution (set the derivative with respect to $\beta$ to zero):
  $$\hat f_\mathrm{ridge}(X) = X^\top \hat \beta_\lambda, \qquad \hat{\beta}_\lambda = (\boldsymbol X^\top \boldsymbol X + \lambda \boldsymbol I)^{-1} \boldsymbol X^\top \boldsymbol Y$$
...
- **The Solution**: Ridge regression constrains coefficients via $\|\beta\|_2^2 \leq s$ (or equivalently adds penalty $\lambda \|\beta\|_2^2$), which shrinks coefficients toward zero and stabilizes the solution with closed form

  $$\hat f_\mathrm{ridge}(X) = X^\top \hat{\beta}_\lambda, \qquad \hat{\beta}_\lambda = (\boldsymbol X^\top \boldsymbol X + \lambda \boldsymbol I)^{-1} \boldsymbol X^\top \boldsymbol Y.$$
...
- **A New Kind of Procedure**: Whereas $\hat f_\mathrm{OLS}$ is the empirical risk minimizer $\hat f_\mathrm{ERM}$, the ridge predictor $\hat f_\mathrm{ridge}$ is *not* — it minimizes empirical risk *plus a penalty*. Ridge is thus our first estimation procedure that departs from ERM, a theme we will return to throughout the course.
