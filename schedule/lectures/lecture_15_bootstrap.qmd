...
However, these methods for uncertaity quantification, variance reduction, and bias reduction all relied on linear structure. Other predictive models do not have such convenient analytic properties.
...
- These confidence intervals tell us that, under our linearity assumptions, the regression function evaluated at $x$ (i.e. $\mathbb E[Y\mid X=x]$ will fall within the interval with 95% probability.
  ...
  - We assume that our training data are **random samples** with $Y = f(X) + \varepsilon$, $f(X) = X^\top \beta$, where $\varepsilon \sim \mathcal{N}(0, \sigma^2)$, for some fixed $\beta, \sigma$.
  ...
  then 95% of those confidence intervals would contain $\mathbb E[Y \mid X=x]$.
  :::
...
However, consider what happens if we use a predictive model that has no clean statistical model (or only an implicit one), like a decision tree or k-nearest neighbors.
...
- Imagine that I have $X_1, \ldots, X_n$ drawn i.i.d. from some distribution $\mathbb{P}_X$.

- The **empirical distribution** $\hat{\mathbb{P}}_X$ is the discrete distribution that places mass $1/n$ at each observed data point $X_i$:

  $$
  \hat{\mathbb{P}}(X=x) = \frac{1}{n} \sum_{i=1}^n \delta_{X_i}(x)
  $$

  where $\delta_{X_i}(x)$ is a point mass at $X_i$.

- If $n$ is large enough, then $\hat{\mathbb{P}}_X \approx \mathbb{P}_X$:
...
:::{.callout-note title="What is the Implication of $\hat{\mathbb{P}}_X \approx \mathbb{P}_X$?"}
- Let's imagine I sample a new point $X^*$ from $\mathbb{P}_X$, and another point $\hat X^{*}$ from $\hat{\mathbb{P}}_X$.
- If $\hat{\mathbb{P}}_X \approx \mathbb{P}_X$, then the distribution of $X^*$ and $\hat X^{*}$ should be similar.
- Thus, sampling from the empirical distribution $\hat{\mathbb{P}}_X$ is (approximately) equivalent to sampling from the true distribution $\mathbb{P}_X$!
:::
...
we can thus sample each $X_i'$ i.i.d. from the empirical distribution $\hat{\mathbb{P}}_X$.
...
:::{.callout-tip title="The Bootstrap"}
  1. Construct the empirical distribution $\hat{\mathbb{P}}_{X,Y}$ based on the observed data points $(X_i, Y_i)$ in $\mathcal D$.
  2. For $b = 1, \ldots, B$:
     - Sample $n$ data points $(X_i^{(b)}, Y_i^{(b)})$ *with replacement* from $\hat{\mathbb{P}}_{X,Y}$ to form dataset $\mathcal D^{(b)}$.
...
- Note that we sample *with replacement* from the empirical distribution because we want each data point in the new dataset to be an independent draw from $\hat{\mathbb{P}}_{X,Y} \approx \mathbb{P}_{X,Y}$.
...
around the regression function $\mathbb E[Y\mid X=x]$ in learning problems,
...
- Recall that we want to generate a confidence interval for our regression function $\mathbb E[Y \mid X=x]$ based on our prediction $\hat Y = \hat f_{\mathcal D}(x)$
...
- We teach this one in DSCI100 because it's easy and "looks right", but it's not theoretically justified in general. It *does* have the right coverage (asymptotically) in some specific cases.
  ...
  <details>
    ...
    If there exists a monotonic function $m: \mathbb R \to \mathbb R$ so that $m(\hat Y)$ is Gaussian distributed, i.e. $m(\hat Y) \sim \mathcal N(m(\hat Y), c^2)$, then the percentile interval has asymptotically correct coverage.
