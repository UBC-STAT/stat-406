...
- In the last lecture, we discussed *boosted ensembles*, which composed simple "weak learners" $h_1, \ldots, h_j$
  ...
  $$ \hat f(X) = \sum_{j=1}^M \beta_j \hat h_j(X), $$
...
- Each weak learner $h_j$ has a set of learnable parameters $\theta_j$ (e.g. split points in trees, location of the "bump"),
  ...
  $$ \hat f(X) = \sum_{j=1}^M \beta_j \hat h(X; \theta_j), $$
...
$$ \hat f(X) = \sum_{j=1}^M \beta_j X^{\theta_j} = \sum_{j=1}^M \beta_j \hat h(X; \theta_j). $$
...
$$ \hat h(X; \theta_j) = g\left( \theta_{j0} + X_1 \theta_{j1} + \ldots + X_{p} \theta_{jp} \right) = g\left( X^\top \theta_j \right)$$
...
- The final prediction is a linear combination of these activations:
  ...
  $$ \hat f(X) = \phi(X)^\top \beta, \quad \beta = \begin{bmatrix} \beta_1 \\ \vdots \\ \beta_M \end{bmatrix}. $$
...
- The model would reduce to a standard linear regression model:
  ...
  $$ \hat f(X) = \sum_{j=1}^M \beta_j (X^\top \theta_j) = X^\top \left( \sum_{j=1}^M \beta_j \theta_j \right). $$
...
- The number of pieces in the piecewise linear function $\hat f(X) = \sum_{j=1}^M \beta_j g(X^\top \theta_j)$ grows with the number of neurons $M$.
