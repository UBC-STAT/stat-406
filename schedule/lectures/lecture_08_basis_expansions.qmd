...
- We have typically assumed the linear statistical model $Y = f(X) + \varepsilon$, where the signal is a linear function of the input features, i.e. $f(X) = X^\top \beta$ for some $\beta \in \mathbb R^p$.
- We will now relax this assumption of linearity, and instead allow the signal to be an arbitrary function:

  $$Y = f(X) + \varepsilon$$

  for some unknown function $f: \mathbb R^p \to \mathbb R$.
- Using a predictive model of the form $\hat f(X) = X^\top \hat \beta$, like what we get from OLS, will not work well if $f$ is highly nonlinear; it will be a high-bias predictor.
...
:::{.callout-important title="But Isn't OLS Unbiased?"}
  ...
  - Under the more general model $Y = f(X) + \varepsilon$, OLS is biased unless $f$ is actually linear.
  - To see why this is the case, let's do a Taylor expansion of $f$ around $0$.

  $$f(X) = f(0) + \nabla f(0)^\top X + \frac{1}{2} X^\top H_f(0) X + \ldots$$

  Even if OLS were to perfectly estimate $\hat \beta = \nabla f(0)$, the higher-order terms would still be missing, and so the linear predictor $\hat f(X) = X^\top \hat\beta$ systematically differs from $f(X)$ (i.e. OLS is biased).
:::
...
:::{.callout-tip collapse="true" title="Quiz: Will Ridge or Lasso Be Beneficial in this Situation?"}
  ...
  - More specifically, Ridge and Lasso still produce predictors of the form $\hat f(X) = X^\top \hat \beta$, which are still missing the higher-order terms that create the bias problem in the first place.
:::
...
- As $d \to \infty$, there exists some set of coefficients $\beta_0, \ldots, \beta_d$ such that $f(x) = \sum_{j=0}^d \beta_j x^j$.
...
- Therefore, we can pretend as if we have a dataset with $d$ features $(X, X^2, \ldots, X^d)$ and use OLS/Ridge/LASSO to learn parameters $\hat \beta_0, \ldots, \hat \beta_d$.
:::
...
- Let's assume that, under our statistical model, $Y = f(X) + \varepsilon$ for some $d^\mathrm{th}$ degree polynomial $f$.
- Let's also assume that we use a $d^\mathrm{th}$ degree polynomial basis expansion to learn a predictor $\hat f(x) = \sum_{j=0}^d \hat \beta_j x^j$ using OLS.
- Then $\hat f(x)$ is an unbiased estimator of $f(x)$!
...
- Thus, the second-order polynomial basis expansion will be of the form:
  ...
  $$\hat f(x) = \hat \beta_0 + \sum_{j=1}^p \hat \beta_j x_j + \sum_{j=1}^p \sum_{k=j}^p \hat \beta_{jk} x_j x_k.$$
...
- We can thus consider a predictive model of the form:
  ...
  $$\hat f(x) = \hat a_0 + \sum_{j=1}^d \hat a_j \cos(j 2 \pi x) + \hat b_j \sin(j 2 \pi x)$$
