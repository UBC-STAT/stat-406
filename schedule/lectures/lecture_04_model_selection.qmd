...
| 3    | Training       | Estimation              | $\hat{\beta}_\mathrm{ERM} = (\boldsymbol{X}^\top \boldsymbol{X})^{-1} \boldsymbol{X}^\top \boldsymbol{Y}$ |
| 4    | Validation      | ???                    | ???                        |
| 5    | Iteration       | ???                    | ???                        |
...
1. **Test error**
    ...
    - Given our training set $\mathcal D = \left\{ (X_i, Y_i) \right\}_{i=1}^n$, the test error is:
      ...
      $$ \widehat{\mathrm{Err}}_\mathcal{D} := \frac{1}{t} \sum_{j=1}^t L(Y_j, \hat f_\mathcal{D}(X_j)) $$

    - I.e. we train our model on the training set $\mathcal D$ and then compute its average loss on the test dataset. We denote the trained model $\hat f_\mathcal{D}$: the hat indicates that it is estimated from data, and the subscript $\mathcal D$ emphasizes that it was estimated using the particular training set $\mathcal D$ (a dependence that is central to model selection as well as the material we will cover in the next lecture).
...
in all cases assuming $\varepsilon \sim \mathcal N(0, \sigma^2)$ for some $\sigma > 0$:

1. **Model 1**: $Y = \beta_0 + \beta_1 X_1 + \varepsilon$
2. **Model 2**: $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \varepsilon$
3. **Model 3**: $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \varepsilon$
4. **Model 4**: $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \beta_4 X_4 + \varepsilon$
...
$$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \beta_4 X_4 + \beta_5 X_1^2 + \varepsilon$$
