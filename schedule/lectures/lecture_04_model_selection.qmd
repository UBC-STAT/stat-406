...
1. Define and differentiate between expected test error and risk
...
We've already covered defining the statistical model, hypothesis class, estimation, and prediction.
...
- Using $\mathcal D$, we performed **empirical risk minimization (ERM)** to obtain a trained model $\hat f_{\mathcal D}$ from our hypothesis class $\mathcal H$ by minimizing the average loss over the training data:
  $$ \hat f_\mathcal{D} := \mathrm{argmin}_{\hat f \in \mathcal H} \; \frac{1}{n} \sum_{i=1}^n L(Y_i, \hat f(X_i)). $$
...
1. **Expected test error**
    ...
    - The **expected test error** is the average loss our trained model $\hat f_\mathcal{D}$ incurs on such a point:
      $$ \mathrm{Err}_\mathcal{D} := \mathbb E \left[ L(Y_\mathrm{new}, \hat f_\mathcal{D}(X_\mathrm{new})) \mid \mathcal D \right] $$
...
2. **Risk**
    ...
    $$ \mathcal R := \mathbb E \left[ L \left( Y_\mathrm{new}, \hat f_{\mathcal D}(X_\mathrm{new}) \right) \right] $$
    ...
    - We can relate $\mathcal R$ to the expected test error via:
      ...
      <details>
        ...
        **Tower rule!!!**
        ...
        \begin{align*}
          \mathcal R &= \mathbb E \left[ L \left( Y_\mathrm{new}, \hat f_{\mathcal D}(X_\mathrm{new}) \right) \right] \\
          &= \mathbb E \left[ \mathbb E \left[ L \left( Y_\mathrm{new}, \hat f_{\mathcal D}(X_\mathrm{new}) \right)  \mid \mathcal D \right] \right] \\
...
and pretend that it was possible to (exactly) compute expected test error and risk.
...
- We could estimate risk by:
    ...
    2. Estimate risk as:
    $$ \hat{\mathcal R} := \frac{1}{m} \sum_{j=1}^m L_j,
    \quad L_j = L\left( Y_{n+1}^{(j)}, \hat f_{\mathcal D^{(j)}}( X^{(j)}_{n+1}) \right),
    ...
    - Because all $n+1$ pairs are i.i.d. from the same distribution, the withheld point $(X_{n+1}, Y_{n+1})$
      is identically distributed to the new point $(X_\mathrm{new}, Y_\mathrm{new})$ from our definition of risk:
      it plays exactly the same role, a fresh draw that the model did not train on.
...
- To reason about cross-validation, it is helpful to make explicit the size of the training set.
  Let $\mathcal R_m$ denote the risk of a model trained on $m$ i.i.d. data points;
  under this notation, the risk of a model trained on our full dataset is $\mathcal R = \mathcal R_n$.
...
**model selection** amounts to choosing the predictive model with the best risk.

:::{.callout-tip title="Model Selection"}
- Given: a single dataset $\mathcal D$
- For however long you have,
    1. Propose a model (hypothesis class + estimation procedure)
...
1. **Model 1**: $f(X) = \beta_0 + \beta_1 X_1$
2. **Model 2**: $f(X) = \beta_0 + \beta_1 X_1 + \beta_2 X_2$
3. **Model 3**: $f(X) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3$
4. **Model 4**: $f(X) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + \beta_4 X_4$

:::{.callout-tip collapse="false" title="Variable Selection is a Specific Instance of Model Selection"}
...
Adding covariates to our regression changes the hypothesis class of predictors we consider.
