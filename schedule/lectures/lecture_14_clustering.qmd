...
- Mathematically, this gives us:
  ...
  $$ \mathrm{argmin}_{\hat f_{\mathcal D}(X)} \frac{1}{2}  \sum_{j=1}^k \vert C_j \vert W_j = \frac{1}{2} \sum_{j=1}^k \frac{1}{\vert C_j \vert} \sum_{i, i' \in C_j} \Vert X_i - X_{i'} \Vert. $$
...
- $f_{\mathcal D}^{(0)}(X)$ will essentially be random:
  ...
  2. Assign each point to the cluster corresponding to the nearest $\mu_j$ value:
    ...
    $$ f_{\mathcal D}^{(0)}(X_i) = \argmin_{j \in \{1, \ldots, k\}} \| X_i - \mu_j \|_2^2. $$
...
In general, there are two key desiderata for our clusters.
...
- As we add more clusters, each cluster will have fewer points. Having fewer points means that points will be closer to one another, so within-cluster variation decreases.
- As we add more clusters, the clusters will be closer to one another, so between-cluster variation also decreases.
</details>
:::
...
- Note that both $B$ and $W$ are functions of $k$, so changing the number of clusters will change both quantities.
