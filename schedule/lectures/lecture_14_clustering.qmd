...
|---------|------------------------------|-------------------------------|
| $\hat f(X): \mathbb R^p \to \mathbb R^k$ | Regression | Dimensionality Reduction |
| $\hat f(X): \mathbb R^p \to \{1, 2, \ldots, k\}$ | Classification | Clustering |
:::
...
- If we apply a clustering algorithm to learn a mapping $\hat f(X): \mathbb R^2 \to \{1, 2, 3\}$, we might get the following result:
...
- **What we are trying to learn**: $\hat f(X) : \mathbb R^p \to \{ 1, \ldots, k \}$, or some function that maps covariates onto clusters.

- For any given $\hat f(X) : \mathbb R^p \to \{ 1, \ldots, k \}$, let $C_1, \ldots, C_k \subset \mathcal D$ represent the **clusters** induced by this mapping; i.e.

  $$C_j = \{ X_i \in \mathcal D : \hat f(X_i) = j \}$$
...
- Mathematically, this gives us:
  ...
  $$ \mathrm{argmin}_{\hat f(X)} \frac{1}{2}  \sum_{j=1}^k \vert C_j \vert W_j = \frac{1}{2} \sum_{j=1}^k \frac{1}{\vert C_j \vert} \sum_{i, i' \in C_j} \Vert X_i - X_{i'} \Vert^2_2. $$

  The fraction $1/2$ is for mathematical convenience (it does not affect the minimization), as will become clear shortly.
...
- Fortunately, we can make use of the following identity to simplify from a pair-wise summation to a per-data-point summation:
  ...
  = \vert C_j \vert^2 \mathbb E_{X, X' \sim \hat{\mathbb P}_{C_j}} \left[ \| (X_i {\color{blue}- \mu_j}) + ({\color{blue}\mu_j -} X_{i'}) \|_2^2 \right]
  $$

  We also note that $\mu_j$ is the expectation of $X$ under the empirical distribution $\hat{\mathbb P}_{C_j}$:

  $$
  \begin{aligned}
  &\vert C_j \vert^2 \mathbb E_{X, X' \sim \hat{\mathbb P}_{C_j}} \left[\Vert (X_i {\color{blue}- \mu_j}) + ({\color{blue}\mu_j -} X_{i'}) \Vert_2^2 \right]
  \\
  =&
  \vert C_j \vert^2 \mathbb E_{X, X' \sim \hat{\mathbb P}_{C_j}} \left[\Vert  (X_i - \mathbb E[X]) + (\mathbb E[X] - X_{i'}) \Vert_2^2 \right]
  \end{aligned}
  $$
...
- We produce a series of clustering assignments
  ...
  $$ \hat f^{(0)}(X), \hat f^{(1)}(X), \hat f^{(2)}(X), \ldots \hat f^{(t)}(X), \ldots $$

  so that $\sum_{j=1}^k |C_j| W_j$ decreases as $t$ increases.

- $\hat f^{(0)}(X)$ will essentially be random:
  ...
  2. Assign each point to the cluster corresponding to the nearest $\mu_j$ value:
    ...
    $$ \hat f^{(0)}(X_i) = \mathrm{argmin}_{j \in \{1, \ldots, k\}} \| X_i - \mu_j \|_2^2. $$

  3. Update $\mu_1, \ldots, \mu_k$ to be the **centroids** of the clusters induced by $\hat f^{(0)}(X)$:

    $$ \mu_j = \frac{1}{|C_j|} \sum_{i \in C_j} X_i. $$

- To get $\hat f^{(1)}(X)$ from $\hat f^{(0)}(X)$, we repeat steps 2 and 3 above.

- More generally, we repeat steps 2 and 3 until the cluster centroids are stable (i.e. when the distance that the cluster centroids change is $< \varepsilon$ for some pre-defined constant $\varepsilon$. Most implementations will define this constant for you.)
...
You can guess what each of these variable names means ;)
:::
...
- As we add more clusters, the between-cluster variation increases!
  (In my opinion, the intuition for this idea is best understood in reverse.
   $B$ essentially measures the empirical variance of our $X_i$ if we were to replace each $X_i$ with its cluster centroid $\mu_{\hat f(X_i)}$.
...
If you want to understand this relationship more rigorously,
try to describe it via the [law of total variance](https://en.wikipedia.org/wiki/Law_of_total_variance).
</details>
:::
...
- K-means alternates between assigning each point to its nearest centroid and updating the centroids to be the mean of the points in each cluster.
