...
3. **Training**: Define a training algorithm (a procedure to choose a $\hat \beta$ from the training data) and apply it to $\mathcal{D}_\mathrm{train}$.
   ...
   - For linear models, we often choose $\hat f$ to minimize the **squared error loss** over training data.
     $$L(Y, \hat Y) = (Y - \hat Y)^2, \qquad \hat \beta = \argmin_{\beta} \frac{1}{n} \sum_{i=1}^{n} L(Y_i, X_i^\top \beta). $$
...
- **Decision theory**: choose a prediction $\hat{Y}_\mathrm{new}$ that minimizes the expected loss:
   ...
   $$\hat{Y}_\mathrm{new} = \argmin_{\hat{y}_\mathrm{new}} \mathbb E[L(Y, \hat{y}_\mathrm{new}) \mid X_\mathrm{new}, \hat \beta].$$
...
- Plugging in the squared error loss function into this formula, we get:
   ...
   $$
   \begin{align*}
      \hat{Y}_\mathrm{new} &= \argmin_{\hat{y}_\mathrm{new}} \mathbb E[(Y_\mathrm{new} - \hat{y}_\mathrm{new})^2 \mid X_\mathrm{new}, \hat \beta] \\
      &= \argmin_{\hat{y}_\mathrm{new}} \left( \underbrace{\mathbb E[Y_\mathrm{new}^2 \mid X_\mathrm{new}, \hat \beta]}_{
      ...
      &= \argmin_{\hat{y}_\mathrm{new}} \left(
...
- We can drop the $\sigma^2$ constant, since it doesn't affect the minimum, and we are left with:
   ...
   $$\hat{Y}_\mathrm{new} = \argmin_{\hat{y}_\mathrm{new}} (\hat{y}_\mathrm{new} - X_\mathrm{new}^\top \hat{\beta})^2,$$
