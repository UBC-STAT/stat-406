...
- We begin by assuming there is some loss $L_{\text{pred}}$ associated with an incorrect prediction.

- A common loss-function for classification is:

   $$
      L_{\text{pred}}(Y_\mathrm{new}, \hat Y_\mathrm{new}) = \begin{cases}
         1 & Y_\mathrm{new} \ne \hat Y_\mathrm{new} \\
         0 & Y_\mathrm{new} = \hat Y_\mathrm{new}
      \end{cases},
   $$

   often referred to as the **0/1 loss**.

:::{.callout-note title="(Important) $L_\text{pred}$ is Theoretical"}
...
- Therefore, this loss is generally not something we compute; it's a theoretical quantity that we can reason about.
- Intuitively, the loss measures how bad our predictions would look in hindsight if eventually we were to find out the true responses associated with our predictions.
:::
...
- Since we don't know the true responses, we instead have to reason about them probabilistically using our estimated $\hat f$:
   ...
   and thus we can think about the *expected loss* of a prediction:

   $$ \mathbb E[ L_{\text{pred}}(Y_\mathrm{new}, \hat Y_\mathrm{new}) \mid X_\mathrm{new}, \hat f]. $$

- According to **decision theory**, the best choice for $\hat Y_\mathrm{new}$ is the one that minimizes expected loss:

   $$ \hat Y_\mathrm{new} = \mathrm{argmin}_{\hat y_\mathrm{new}} \mathbb E[ L_{\text{pred}}(Y_\mathrm{new}, \hat y_\mathrm{new}) \mid X_\mathrm{new}, \hat f]. $$

- Perhaps unsurprisingly, under the 0/1 loss, this decision simplifies down to:
   ...
   <details>
      ...
      The expected 0/1 loss of predicting $\hat y_\mathrm{new}$ is a sum over the two possible outcomes, each weighted by its probability under our model:
      \begin{align*}
      \hat{Y}_\mathrm{new}
      ...
      \end{align*}
      We should therefore predict $\hat y_\mathrm{new} = 1$ whenever doing so incurs the smaller expected loss, i.e. whenever $\hat \pi_0(X_\mathrm{new}) \leq \hat \pi_1(X_\mathrm{new})$. Since $\hat \pi_0 = 1 - \hat \pi_1$, this is exactly the condition $\hat \pi_1(X_\mathrm{new}) \geq 0.5$. Finally, recall that $\hat \pi_1(X_\mathrm{new}) = \sigma(\hat f(X_\mathrm{new})) \geq 0.5$ if and only if $\hat f(X_\mathrm{new}) \geq 0$.
   </details>
...
### How Prediction Loss Affect Decisions

- While the derivation above was perhaps a complicated way to obtain a simple prediction rule,
   decision theory shows its power when we start considering alternative loss functions.
...
- For example, imagine a world where **false-positive predictions** (predicting $\hat Y = 1$ when $Y=0$) were twice as bad as **false-negative predictions** (predicting $\hat Y = 0$ when $Y=1$).
   ...
   - We could codify this idea into a loss function:
   ...
   - Now considering our decision theoretic rule:
      ...
      the $\hat f$ estimate is the same as above but now the loss function $L_{\text{pred}}$ is different. Thus we should end up with a different rule for $\hat Y_\mathrm{new}$.
   ...
   - Turning the crank, we have:
      ...
      <details>
         <summary>Derivation</summary>
         The expected loss of the two possible predictions are
         \begin{align*}
            \mathbb E[ L_{\text{pred}}(Y_\mathrm{new}, 1) \mid X_\mathrm{new}, \hat f] &= 2 \, \hat \pi_0(X_\mathrm{new}) & \text{(loss 2 incurred only if } Y_\mathrm{new} = 0), \\
            \mathbb E[ L_{\text{pred}}(Y_\mathrm{new}, 0) \mid X_\mathrm{new}, \hat f] &= 1 \, \hat \pi_1(X_\mathrm{new}) & \text{(loss 1 incurred only if } Y_\mathrm{new} = 1).
         \end{align*}
         We predict $\hat Y_\mathrm{new} = 1$ whenever its expected loss is no larger, i.e. whenever
         \begin{align*}
...
- In general, we can encode any weighting of false positives versus false negatives into the loss, and the resulting rule is just a threshold on $\hat \pi_1(X_\mathrm{new})$ (equivalently, on $\hat f(X_\mathrm{new})$) that shifts with the relative loss
...
4. **Prediction**: The optimal prediction rule depends on the prediction loss function, $L_{\text{pred}}$.

5. **Linear Decision Boundary**: Under the $0/1$ loss for prediction, the $Y=0$ and $Y=1$ predictions are separated by a hyperplane defined by the decision boundary $X^\top \hat \beta = 0$.
