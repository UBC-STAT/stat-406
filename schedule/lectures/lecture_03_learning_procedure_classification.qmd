...
| 3 | Training |  Estimation | $\hat \beta_\mathrm{OLS} = \hat \beta_\mathrm{ERM} = \hat \beta_\mathrm{MLE} = (\boldsymbol X^\top \boldsymbol X)^{-1} \boldsymbol X^\top \boldsymbol Y $
| 6 | Testing (Inference) | Prediction | $\hat Y_\mathrm{new} = X_\mathrm{new}^\top \hat \beta$ |
...
$$\hat{Y}_{\text{new}} = \argmin_{\hat{y}} \mathbb{E}[L(Y, \hat{y}) \mid X_{\text{new}}, \hat \beta]$$
...
$$
\begin{align*}
\hat{Y}_{\text{new}} &= \argmin_{\hat{y}} \mathbb{E}[ \mathbb{I}(Y \neq \hat{y}) \mid X_{\text{new}}, \hat \beta] \\
&= \argmin_{\hat{y}} P(Y \neq \hat{y} \mid X_{\text{new}}, \hat \beta) \\
&= \argmin_{\hat{y}} \left[
...
$$\hat{\beta}_\mathrm{MLE} = \argmax_{\beta} \mathcal L(\beta) = \argmax_{\beta} \ell(\beta)$$
...
$$
\hat \beta_\mathrm{MLE} = \argmin_{\beta} \sum_{i=1}^n \left[ Y_i \log(\sigma(X_i^\top \beta)) + (1 - Y_i) \log(1 - \sigma(X_i^\top \beta)) \right]
$$
...
$$\hat{\beta}_\mathrm{MLE} = \argmin_{\beta} \sum_{i=1}^n L(Y_i, \hat Y_i)$$
...
$$\argmin_{\beta} \sum_{i=1}^n \mathbb{I}(Y_i \neq \mathbb{I}(X_i^\top \beta > 0))$$
...
$$\hat{\beta}_\mathrm{ERM} = \argmin_{\beta} \sum_{i=1}^n \left[ (1-Y_i) \sigma(X_i^\top \beta) + Y_i (1 - \sigma(X_i^\top \beta)) \right]$$
...
\begin{align*}
\hat{\beta}_\mathrm{ERM} &= \argmin_{\beta} \sum_{i=1}^n \left[ (1-Y_i) \sigma(X_i^\top \beta) + Y_i (1 - \sigma(X_i^\top \beta)) \right] \\
&= \argmax_{\beta} \sum_{i=1}^n \left[ (Y_i - 1) \log(\sigma(X_i^\top \beta)) + (-Y_i) \log(1 - \sigma(X_i^\top \beta)) \right] \\
&= \argmax_{\beta} \sum_{i=1}^n \left[ Y_i \log(\sigma(X_i^\top \beta)) + (1-Y_i) \log(1 - \sigma(X_i^\top \beta)) \right]
