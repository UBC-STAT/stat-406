...
- In classification, we are interested in families of conditional distributions $\mathbb{P}_{Y \mid X}$. For binary classification, each such distribution is a Bernoulli that is fully specified by the probability $\mathbb{P}(Y=1 \mid X = x)$.
   - This probability implicitly defines $\mathbb{P}(Y=0 \mid X = x)$ by the law of total probability, i.e. $\mathbb{P}(Y=0 \mid X = x) = 1 - \mathbb{P}(Y=1 \mid X = x)$.
- **Notation**: For simplicity, let's define:
   - $\pi_1(x) := \mathbb{P}(Y = 1 \mid X = x)$ (probability of positive class)
   - $\pi_0(x) := \mathbb{P}(Y = 0 \mid X = x)$ (probability of negative class)
...
- You may remember from STAT 406 or CPSC 340 a set of distributions that look like:
   $$\mathbb{P}(Y = 1 \mid X = x) = \pi_1(x) = \frac{\exp(x^\top \beta)}{1 + \exp(x^\top \beta)}$$
...
- Instead of defining distributions through $\mathbb{P}(Y = 1 \mid X = x)$ directly, we will instead define distributions through the **log-odds ratio**
...
$$\left\{ \mathbb{P}_{Y \mid X} \: : \: \mathbb{P}(Y=1 \mid X=x) = \frac{\exp(x^\top\beta)}{1 + \exp(x^\top\beta)}, \quad \beta \in \mathbb{R}^p \right\}$$
...
- Our statistical model is then the set of conditional distributions $\mathbb{P}_{Y \mid X}$ whose class-1 probability $\mathbb{P}(Y=1 \mid X=x)$ takes the form of $\pi_1$.
:::
...
&= \mathrm{argmin}_{\hat{y}} \mathbb{P}(Y \neq \hat{y} \mid X_{\text{new}}, \hat \beta) \\
&= \mathrm{argmin}_{\hat{y}} \left[
   \underbrace{\mathbb{P}(Y = 1 \mid X_{\text{new}}, \hat \beta)}_{\sigma(X_\mathrm{new}^\top \hat \beta)} \mathbb{I}(\hat{y} = 0) +
   \underbrace{\mathbb{P}(Y = 0 \mid X_{\text{new}}, \hat \beta)}_{1 - \sigma(X_\mathrm{new}^\top \hat \beta)} \mathbb{I}(\hat{y} = 1) \right] \\
&= \begin{cases}
...
1. **Probabilistic loss**: $L_\mathrm{prob}(Y, \hat{Y}) = -\log \mathbb{P}(Y = \hat Y)$
...
- Given training data $\mathcal{D} = \{(X_1, Y_1), \ldots, (X_n, Y_n)\}$, recall that the likelihood/log-likelihood function is:
   ...
   \begin{gather}
   \mathcal{L}(\beta) = \prod_{i=1}^n \mathbb{P}(Y_i \mid X_i; \beta) \\
   \ell(\beta) = \sum_{i=1}^n \log \mathbb{P}(Y_i \mid X_i; \beta)
   \end{gather}

- Plugging in our model, we have that:

   $$
   \mathbb{P}(Y_i \mid X_i; \beta) = \begin{cases}
...
- We can write this equation more compactly as:
   ...
   $$\mathbb{P}(Y_i \mid X_i, \beta) = \sigma(X_i^\top \beta)^{Y_i} (1 - \sigma(X_i^\top \beta))^{1-Y_i}.$$
...
(and recognizing that $\mathbb{E}[Y \mid X_{\text{new}}, \hat \beta] = \mathbb{P}(Y=1 \mid X_{\text{new}}, \hat \beta)$).
