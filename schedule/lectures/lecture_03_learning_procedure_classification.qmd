...
$$\hat{\beta}_\mathrm{ERM} = \mathrm{argmin}_{\beta} \sum_{i=1}^n L(Y_i, \hat Y_i)$$
...
- This optimization problem not only has no closed-form solution, but numerical solutions to it are intractable to compute.
...
- A little manipulation will show that this is equivalent to the MLE optimization problem!

:::{.callout-info collapse="true" title="Derivation"}
\begin{align*}
\hat{\beta}_\mathrm{ERM} &= \mathrm{argmin}_{\beta} \sum_{i=1}^n \left[ (1-Y_i) \sigma(X_i^\top \beta) + Y_i (1 - \sigma(X_i^\top \beta)) \right] \\
