...
- A **weak learner** is a predictive model that can only achieve slightly better than random guessing on any learning problem.
  ...
  a weak learner can achieve $50\% + \varepsilon$ accuracy for some small $\varepsilon > 0$.
...
- We then simultaneously select $\beta^{(t)}$ and $\hat h^{(t)}$ to minimize the squared error on these residuals
  ...
  $$
  \hat h^{(t)}, \beta^{(t)} = \mathrm{argmin}_{h, \beta} \sum_{i=1}^n (R_i^{(t)} - \beta h(X_i))^2
  $$
...
- The above procedure can be generalized to arbitrary loss functions $L(Y, \hat f(X))$ (not just squared error).
...
- For the squared error, note that:
  ...
  &= L( Y_i, \hat f^{(t-1)}(X_i) + \beta^{(t)} \hat h^{(t)}(X_i) )
  \end{aligned}
  $$
...
- Therefore, we can train the weak learners to minimize the loss on the updated ensemble predictions directly:
  ...
  $$
  (\hat h^{(t)}, \beta^{(t)}) = \mathrm{argmin}_{h, \beta} \sum_{i=1}^n L( Y_i, \hat f^{(t-1)}(X_i) + \beta h(X_i) )
  $$
...
- Choose a weak learner $\hat h^{(t)}$ that fits the current residuals:
   ...
   $$
   \hat h^{(t)}, \beta^{(t)} = \mathrm{argmin}_{h, \beta} \sum_{i=1}^n L \left( Y_i, \hat f^{(t-1)}(X_i) + \beta h(X_i) \right)
   $$
...
- The **exponential loss** is given by:
  ...
  - If $Y$ and $\hat f(X)$ have the same sign (correct classification), then the term inside the exponential is negative (low loss)
  - If $Y$ and $\hat f(X)$ have opposite signs (incorrect classification), then the term inside the exponential is positive (high loss)
  - We can view the exponential loss as an approximation to the 0-1 loss that **really** penalizes large/incorrect $\hat f(X)$ values.
...
- Let's apply this loss (and the fact that $\hat h^{(t)}(X_i) \in \{-1, +1\}$) to the generic boosting optimization problem:
  ...
  (\hat h^{(t)}, \beta^{(t)}) &= \mathrm{argmin}_{h, \beta} \sum_{i=1}^n L \left( Y_i, \hat f^{(t-1)}(X_i) + \beta h(X_i) \right)
  \\
  &= \mathrm{argmin}_{h, \beta} \sum_{i=1}^n \exp\left( -Y_i \left( \hat f^{(t-1)}(X_i) + \beta h(X_i) \right) \right)
  \\
  &= \mathrm{argmin}_{h, \beta} \sum_{i=1}^n \underbrace{\exp\left( -Y_i \hat f^{(t-1)}(X_i) \right)}_{W_i} \exp\left( -\beta Y_i h(X_i) \right)
  \end{align*}
  $$
...
- We can thus subdivide the above summation into points that $h$ correctly classifies and points that it misclassifies:
  ...
  $$
  \begin{align*}
  (\hat h^{(t)}, \beta^{(t)}) &= \mathrm{argmin}_{h, \beta} \left[ \sum_{i: Y_i = h(X_i)} W_i \exp \left( -\beta \underbrace{Y_i h(X_i)}_{=+1} \right) + \sum_{i: Y_i \neq h(X_i)} W_i \exp \left( -\beta \underbrace{Y_i h(X_i)}_{=-1} \right) \right] \\
  &= \mathrm{argmin}_{h, \beta} \left[ \exp(-\beta) \sum_{i: Y_i = h(X_i)} W_i + \exp(\beta) \sum_{i: Y_i \neq h(X_i)} W_i \right]
  \end{align*}
  $$
...
- By inspection (and assuming that $\beta > 0$), we can see that the optimal weak learner $\hat h^{(t)}$ is the one that minimizes the **weighted classification error**:
  ...
  $$
  \hat h^{(t)} = \mathrm{argmin}_h \sum_{i=1}^n W_i \mathbb{I}(Y_i \neq h(X_i))
  $$
...
- These residuals can be viewed as the **negative gradient** of the squared loss function:
  ...
  $$
- Gradient boosting generalizes this idea to any differentiable loss function $L (Y, \hat f(X))$ by training each weak learner to predict the **negative gradient** of the loss:
  $$
  G_i^{(t)} = -\frac{\partial L(Y_i, \hat f(X_i))}{\partial \hat f(X_i)} \bigg|_{\hat f = \hat f^{(t-1)}}
  $$
...
- A differentiable loss function $L(Y, \hat f(X))$
- A weak learning algorithm (e.g., depth-2 decision trees)
- The number of boosting rounds $m$

Initialize: Set $\hat f^{(0)}(X) = \mathrm{argmin}_c \sum_{i=1}^n L(Y_i, c)$ (constant initial prediction)

For $t = 1, 2, \ldots, m$:

1. Compute the negative gradient (pseudo-residuals) for each training example:
   $$
   G_i^{(t)} = -\frac{\partial L(Y_i, \hat f^{(t-1)}(X_i))}{\partial \hat f^{(t-1)}(X_i)}
   $$
2. Train a weak learner $\hat h^{(t)}$ to predict the pseudo-residuals:
   $$
   \hat h^{(t)} = \mathrm{argmin}_h \sum_{i=1}^n L (G_i^{(t)}, h(X_i))
   $$
3. Find the optimal weight for the weak learner:
   $$
   \beta^{(t)} = \mathrm{argmin}_\beta \sum_{i=1}^n L(Y_i, \hat f^{(t-1)}(X_i) + \beta \hat h^{(t)}(X_i))
   $$
...
- In Step 2 of the algorithm, our goal is to minimize the loss between the gradients and the weak learner predictions:
  ...
  $$ \hat h^{(t)} = \mathrm{argmin}_h \sum_{i=1}^n \left( G_i^{(t)} - h(X_i) \right)^2, $$
...
- Minimizing the cosine similarity is thus equivalent to:
  ...
  \\
  &= \min_{\hat h^{(t)}} \sum_{i=1}^n L(Y_i, \hat f^{(t-1)}(X_i)) + \langle \boldsymbol G^{(t)}, \boldsymbol{\hat h}^{(t)} \rangle
  \end{align*}
  $$

  where the last two equalities hold because neither $\Vert \boldsymbol G^{(t)} \Vert$ nor $\sum_{i=1}^n L(Y_i, \hat f^{(t-1)}(X_i))$ depend on $\hat h^{(t)}$.
...
- In other words, Step 2 is equivalent to finding $\hat h^{(t)}$ that minimizes the first-order Taylor approximation of the loss around the current ensemble predictions:
  ...
  $$
  \sum_{i=1}^n L(Y_i, \hat f^{(t-1)}(X_i) + \hat h^{(t)}(X_i)) \approx \sum_{i=1}^n L(Y_i, \hat f^{(t-1)}(X_i)) + \langle \boldsymbol G^{(t)}, \boldsymbol{\hat h}^{(t)} \rangle.
  $$
