...
bind_rows(df, df1) %>%
  ...
  ggplot(aes(x = l1norm, y = coefficient, color = predictor)) +
  geom_path() +
...
[Lasso is **not** a linear smoother]{.secondary}. There is no matrix $S$ such that $\widehat{y} = Sy$ for the predicted values from lasso.

* We can't use `cv_nice()`.

* We don't have $\tr{S} = \textrm{df}$ because [there is no $S$]{.secondary}.
