...
__Regression:__  $\hat{R}_i = \frac{1}{2}(y_i - \hat{y}_i)^2$ (the 1/2 just makes the derivitave nice)
...
$$\begin{cases}
\hat{\mathbf{y}} = B^\top \mathbf{A}^{(T)} \in \R^n \\
\hat{\Pi} = \left(1 + \exp\left(-\mathbf{BA}^{(T)}\right)\right)^{-1} \in \R^{n \times M}\end{cases}$$
