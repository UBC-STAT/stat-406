...
then it is better to predict with `r round(aopt(mu,n),2)` $\overline{Y}_n$ 

than with $\overline{Y}_n$ itself.  

--

For this $a =$ `r round(aopt(mu,n),2)`

1. $R(a)=R_1(a\overline{Y}_n) =$ `r round(risk(aopt(mu,n),mu,n),2)`
...
`r fa("bomb", fill=red)` __This is hard:__  Don't know $P$ (if I knew the truth, this would be easy)
...
more complicated $\mathcal{F}$, lower bias. Flexibility $\Rightarrow$ Expressivity
