...
= \cdots = \cdots\\
&= x^\top\hat\Sigma^{-1}\overline X_1-\frac{1}{2}\overline X_1^\top \hat\Sigma^{-1}\overline X_1 + \log \hat\pi_1 - x^\top\hat\Sigma^{-1}\overline X_0-\frac{1}{2}\overline X_0^\top \hat\Sigma^{-1}\overline X_0 + \log \hat\pi_0\\
&=: \delta_1(x) - \delta_0(x)
\end{aligned}
$$
...
We just calculate $(a_1 - a_0)$ (a vector in $\R^p$), and $b_1 - b0$ (a scalar)
...
library(mvtnorm)
library(MASS)
generate_lda <- function(n, p = c(.5,.5), 
