...
fun <- function(n = 812) {
  ...
  train <- df[1:ceiling(n / 2), ] 
  test <- df[(ceiling(n / 2) + 1):n, ]
  ...
  out <- c(
    correct = mean( (test$y - predict(truth, newdata = test))^2 ),
...
mses <- sim %>% 
  ...
  mutate(full = (full - truth) / truth * 100,
         ...
         correct = (correct - truth) / truth * 100, truth=NULL)
g1 = mses %>% 
  ...
  ylab("% increase in mse relative to oracle")
g2 = sim %>% 
  ggplot(aes(y = nbest)) + 
