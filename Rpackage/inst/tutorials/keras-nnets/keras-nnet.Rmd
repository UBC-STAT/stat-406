...
* MacOS
* R/Rstudio 4.0+
* Existing installation of Miniconda3
...
reticulate::use_condaenv(condaenv = "r-reticulate",
                         ...
                         required = TRUE)
library(tensorflow)
tf$constant("Hellow Tensorflow") ## checks that it points to the right thing
```
...
<iframe src="https://www.facebook.com/plugins/post.php?href=https%3A%2F%2Fwww.facebook.com%2Fyann.lecun%2Fposts%2F10157251192782143&show_text=true&width=552&height=576&appId" width="552" height="576" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>
...
![Figure 1. Fashion-MNIST samples (by Zalando, MIT License).](https://github.com/zalandoresearch/fashion-mnist/blob/master/doc/img/fashion-mnist-sprite.png)
...
```{r show-some-clothes, exercise=TRUE}
par(mfcol=c(5,5))
...
```{r train-score, exercise=TRUE}
train_score <- model %>% evaluate(train_images, train_labels, verbose=0)
train_score
```

```{r train-score-keep, echo=FALSE}
train_score <- model %>% evaluate(train_images, train_labels, verbose=0)
```


As the model trains, the loss and accuracy metrics are displayed. This model reaches an accuracy of about `r round(train_score[2],2)` on the training data.

### Evaluate accuracy

Next, compare how the model performs on the test dataset:

```{r test-score, exercise=TRUE}
...
```{r top20class-preds, exercise=TRUE}
class_pred[1:20]
```
...
```{r pred-one-image, exercise=TRUE, exercise.setup="one-image"}
...
```{r one-pred, exercise=TRUE, exercise.setup="pred-one-image"}
prediction <- predictions[1, ]
which.max(prediction) - 1 # subtract 1 as labels are 0-based
```

Or, directly getting the class prediction again:

```{r class-pred, exercise=TRUE, exercise.setup="one-image"}
