...
my_forest = randomForest(lobes ~. -Species-Specimen_Number, 
                         data = leaf, ntree=400)
m = ncol(leaf)-3
my_bagged_trees = randomForest(lobes ~. -Species-Specimen_Number, 
                               data = leaf, ntree=400, mtry=m)
```
...
* Begin by producing a pairs plot of all continuous predictors. 
* Color the points by `lobes`. 

_Be patient as it may take some time to run._

```{r visualplot, exercise=TRUE, exercise.timelimit = 60}
leaf <- UBCstat406labs::leaf
library(GGally)
...
1. Train a tree classifier based on this data for predicting complexity. 
1. Use all predictors (not `Species` or `Specimen_number` obvs). 
1. Prune your tree using cross validation to choose the depth and plot the tree.
...
1. Train a random forest using 400 trees. 
1. Produce a variable importance plot, a confusion matrix, and find the out-of-bag error rate.
...
Produce 
1. a variable importance plot, 
1. a confusion matrix, 
1. and find the in-sample error rate.

```{r forest-eval, exercise=TRUE}
...
1. Perform bagging using 400 trees. 
1. Produce a variable importance plot, a confusion matrix, and find the out-of-bag error rate.

```{r baggedpredictions, exercise=TRUE, message=FALSE}
m = ncol(leaf)-3
my_bagged_trees = randomForest(lobes ~. -Species-Specimen_Number, 
                               data = leaf, ntree=400, mtry=m)
```
...
```{r bagged-eval, exercise=TRUE}
varImpPlot(my_bagged_trees)
(bagged_conf <- table(predict(my_bagged_trees), leaf$lobes))
1 - sum(diag(bagged_conf))/sum(bagged_conf)
```


## Discussion points

* Out-of-bag error vs CV vs training error
* Interpretability
* Tuning parameters?
* Intuition for bias vs variance in RFs vs Bagging
