...
**NOTE:** for 2026/27 Winter1, please disregard set-up instructions for Git and Github, as we will not use them this term.
...
## Note about unavailable R packages

If at any point you require R package that is not currently available on CRAN, (for example, the ‘ElemStatLearn’ package: https://cran.r-project.org/web/packages/ElemStatLearn/index.html), the following set of instructions may be useful.

1. Navigate to where the package is archived (for example: "http://cran.r-project.org/src/contrib/Archive/ElemStatLearn/")
2. Identity the version you want to install (for example: "ElemStatLearn_2015.6.26.2.tar.gz")
3. With the above information, you should be able to install the archived package by code similar to the following:

```R
# This code installs a specific archived version of package `ElemStatLearn' 
...
4. Verify that the code was installed successfully by loading the library in R.
